...
It is actually not only a protocol, but also a set of related projects:
- [b-code-spec] : the specification of the protocol itself
- [b-code-desc] : a JSON description format of the capabilities of a robot (possible actions, sensor list, id card) to make it easier to be controlled by generic and reusable remote controls, or even LLMs
- [b-code-remote-control] : a web-based remote control for robots supporting b-code and having a b-code description file
- [b-code-arduino-interpreter] : an Arduino library to interpret and execute b-code commands
...
## Installation
This firmware is developed with PlatformIO. The recommanded way to build and install it is to use Visual Studio Code with the PlatformIO extension.
